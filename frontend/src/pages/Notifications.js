        ...
        {loading ? (
          ...
          <Card className="text-center py-5 border-0 shadow-sm rounded-4">
            <Card.Body>
              ...
              <Button variant="dark" className="rounded-pill px-4" onClick={() => navigate('/menu')}>Explore Menu</Button>
            </Card.Body>
          </Card>
        ) : (
          messages.map((msg) => (
            <div key={msg._id} className="notification-card bg-white shadow-sm mb-4 rounded-4 overflow-hidden">
              <div className="notification-header bg-light">
                <div>
                  <span className="notification-id text-dark fw-bold me-2">#{msg.orderId?._id?.slice(-8).toUpperCase() || 'SYSTEM'}</span>
              ...
              <div className="notification-body bg-white p-3">
                <div className="notification-step border-bottom-0 pb-0 mb-0">
                   <div className="notification-icon bg-dark bg-opacity-10 text-dark rounded-circle d-flex align-items-center justify-content-center" style={{ width: '40px', height: '40px' }}>
