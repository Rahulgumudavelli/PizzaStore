...
import { Container, Row, Col, Card, Button, Table, Toast, ToastContainer, Form, Badge, Modal } from 'react-bootstrap';
import { useNavigate } from 'react-router-dom';
import { getCart, createOrder, getAddresses, addAddress, updateCartQuantity, removeFromCart } from '../services/api';
...
const Cart = () => {
  ...
  const handleUpdateQuantity = async (itemId, currentQty, delta) => {
    ...
    if (newQty < 1) return;
    try {
      await updateCartQuantity({ itemId, quantity: newQty });
      fetchCart();
    } catch (error) {
  ...
  return (
    <div className="d-flex flex-column min-vh-100 bg-light">
      ...
      <ToastContainer position="top-center" className="p-3" style={{ zIndex: 1050 }}>
        <Toast bg="dark" show={showToast} delay={2000} autohide onClose={() => setShowToast(false)}>
          <Toast.Body className="text-white fw-bold d-flex justify-content-center align-items-center py-3">
            <i className="bi bi-check-circle-fill me-2 fs-5"></i> Order Placed Successfully!
          </Toast.Body>
        </Toast>
      </ToastContainer>
        ...
        {loading ? (
          ...
          <Card className="text-center py-5 border-0 shadow-sm rounded-4">
            <Card.Body>
              ...
              <Button variant="dark" className="rounded-pill px-5 py-2 fw-bold"
            ...
            <Col lg={8}>
                  ...
                  <tbody>
                    {cart.items.map(item => (
                      <tr key={item._id} className="align-middle">
                        <td className="py-3 px-4 fw-bold border-0">{item.name}</td>
                        <td className="py-3 text-center text-muted border-0">₹{item.price}</td>
                        <td className="py-3 text-center border-0">
                          <div className="d-flex align-items-center justify-content-center gap-2">
                             <Button 
                               variant="outline-secondary" size="sm" className="rounded-circle p-0" 
                               style={{ width: 24, height: 24, lineHeight: 1 }}
                               onClick={() => handleUpdateQuantity(item.itemId?._id || item.itemId, item.quantity, -1)}
                               disabled={item.quantity <= 1}
                             >
                               -
                             </Button>
                             <span className="fw-bold mx-1" style={{ minWidth: '20px' }}>{item.quantity}</span>
                             <Button 
                               variant="outline-secondary" size="sm" className="rounded-circle p-0" 
                               style={{ width: 24, height: 24, lineHeight: 1 }}
                               onClick={() => handleUpdateQuantity(item.itemId?._id || item.itemId, item.quantity, 1)}
                        ...
                        <td className="py-3 text-end px-4 fw-bold text-dark border-0">₹{item.price * item.quantity}</td>
                        <td className="py-3 text-center border-0">
                          <button 
              ...
              <Card className="border-0 shadow-sm rounded-4 mb-4">
                ...
                <div className="btn-group btn-group-sm flex-wrap">
                    <Button 
                      variant={deliveryMode === 'delivery' ? 'dark' : 'outline-dark'}
                      onClick={() => setDeliveryMode('delivery')}
                    >
                      Delivery
                    </Button>
                    <Button 
                      variant={deliveryMode === 'pickup' ? 'dark' : 'outline-dark'}
                  ...
                  {deliveryMode === 'delivery' ? (
                    <div className="mt-2">
                      {addresses.length > 0 ? (
                        <Row className="g-3">
                          {addresses.map(addr => (
                            <Col md={6} key={addr._id}>
                              <Card 
                                className={`h-100 border-2 cursor-pointer rounded-4 transition-all ${selectedAddressId === addr._id ? 'border-dark bg-dark bg-opacity-10' : 'border-light shadow-sm'}`}
                                ...
                                <Card.Body className="p-3">
                                  <div className="d-flex justify-content-between align-items-start mb-2">
                                    <h6 className="fw-bold mb-0">
                                      {addr.isDefault && <Badge bg="dark" className="me-2 rounded-pill">Default</Badge>}
                                      Address
                                    </h6>
                                    {selectedAddressId === addr._id && <i className="bi bi-check-circle-fill text-dark"></i>}
                                  </div>
                        ...
                        <div className="text-center py-4 bg-light rounded-4">
                          ...
                          <Button variant="dark" className="rounded-pill px-4" onClick={() => setShowAddressModal(true)}>
                    ...
                    <div className="text-center py-4 bg-light rounded-4">
                      ...
                      <p className="small text-muted mb-0">
                        ...
                        <span className="text-dark fw-bold">Available for pickup in 20-30 mins</span>
                      </p>
                    </div>
                  )}
                </Card.Body>
              ...
              <Card className="border-0 shadow-sm rounded-4 mb-4">
                ...
                <Card.Body className="pt-0">
                  <Row className="g-3">
                    <Col md={6}>
                      <Card 
                        className={`h-100 border-2 rounded-4 transition-all ${paymentMethod === 'cod' ? 'border-dark bg-dark bg-opacity-10' : 'border-light shadow-sm'}`}
                        onClick={() => setPaymentMethod('cod')}
                        style={{ cursor: 'pointer' }}
                      >
                        <Card.Body className="p-3 d-flex align-items-center gap-3">
                          <div className={`rounded-circle p-2 ${paymentMethod === 'cod' ? 'bg-dark text-white' : 'bg-light text-muted'}`}>
                    ...
                    <Col md={6}>
                      <Card 
                        className={`h-100 border-2 rounded-4 transition-all ${paymentMethod === 'card' ? 'border-dark bg-dark bg-opacity-10' : 'border-light shadow-sm'}`}
                        onClick={() => setPaymentMethod('card')}
                        style={{ cursor: 'pointer' }}
                      >
                        <Card.Body className="p-3 d-flex align-items-center gap-3">
                          <div className={`rounded-circle p-2 ${paymentMethod === 'card' ? 'bg-dark text-white' : 'bg-light text-muted'}`}>
              ...
              <Card className="border-0 shadow-lg rounded-4 overflow-hidden sticky-top" style={{ top: '90px', zIndex: 10 }}>
                <Card.Body className="p-4">
                  ...
                  <Button
                    ...
                    disabled={ordering || (deliveryMode === 'delivery' && !selectedAddressId)}
                  >
                    {ordering ? (
